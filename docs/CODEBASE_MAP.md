...
| Web-search tool gating | [`backend/qwythos/utils/tools.py`](../backend/qwythos/utils/tools.py) `get_builtin_tools()` — requires admin `ENABLE_WEB_SEARCH` + a configured search engine, per-model capability, and user permission |
| Default search engine | [`backend/qwythos/config.py`](../backend/qwythos/config.py) — `ENABLE_WEB_SEARCH` defaults `True` and `WEB_SEARCH_ENGINE` defaults `duckduckgo` (via [`retrieval/web/duckduckgo.py`](../backend/qwythos/retrieval/web/duckduckgo.py), the `ddgs` package), the only engine needing neither an API key nor a separate service. Every other engine (Tavily, Brave, SearXNG, ...) still needs admin-provided credentials/URL and overrides this default. **Only takes effect on a fresh `backend/data` DB** — an already-initialized install keeps whatever it persisted on first boot; flip it in Admin Settings → Web Search there instead. |
