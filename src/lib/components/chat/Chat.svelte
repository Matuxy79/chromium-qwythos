<script lang="ts">
	...
	import QwythosStackBackground from './QwythosStackBackground.svelte';
			...
			<PaneGroup direction="horizontal" class="w-full h-full">
				<Pane defaultSize={50} minSize={30} class="h-full flex relative max-w-full flex-col">
					...
					<div id="chat-pane" class="flex flex-col flex-auto z-10 w-full @container overflow-auto">
						...
						{:else}
							<div class="relative isolate flex items-center h-full overflow-hidden">
								{#if !($settings?.backgroundImageUrl ?? $config?.license_metadata?.background_image_url ?? null)}
									<QwythosStackBackground />
								{/if}
