			...
			<AdminSettingSection title={$i18n.t('User Access')} first>
				...
				{#if adminConfig?.ENABLE_API_KEYS}
					...
					{#if adminConfig?.ENABLE_API_KEYS_ENDPOINT_RESTRICTIONS}
						...
						>
							...
							/>
							<a
								href="https://docs.qwythos.com/reference/api-endpoints"
								target="_blank"
				...
				>
					...
					{#if adminConfig.JWT_EXPIRES_IN === '-1'}
						<a
							href="https://docs.qwythos.com/reference/env-configuration#jwt_expires_in"
							target="_blank"
		...
		<AdminSettingSection title={$i18n.t('LDAP')}>
			...
			{#if ENABLE_LDAP}
				...
				<AdminSettingRow
					...
					description={$i18n.t('Map LDAP groups to Qwythos groups.')}
					let:labelId
				>
			...
			<AdminSettingSection title={$i18n.t('OAuth / OIDC')}>
				...
				{#if oauthConfig.ENABLE_OAUTH}
					...
					<AdminSettingRow
						...
						description={$i18n.t('Map OAuth claims to Qwythos roles.')}
						let:labelId
					>
						<Switch
					...
					<AdminSettingRow
						label={$i18n.t('Group Mapping')}
						description={$i18n.t('Map OAuth claims to Qwythos groups.')}
						let:labelId
					>
						<Switch
