<script>
	...
	const saveHandler = async (data) => {
		...
		if (compareVersion(manifest?.required_qwythos_version ?? '0.0.0', WEBUI_VERSION)) {
			console.log('Version is lower than required');
			toast.error(
				$i18n.t(
					'Qwythos version (v{{QWYTHOS_VERSION}}) is lower than required version (v{{REQUIRED_VERSION}})',
					{
						QWYTHOS_VERSION: WEBUI_VERSION,
						REQUIRED_VERSION: manifest?.required_qwythos_version ?? '0.0.0'
					}
				)
			);
			return;
	...
	onMount(() => {
		...
		window.addEventListener('message', async (event) => {
			if (
				!['https://qwythos.com', 'https://www.qwythos.com', 'http://localhost:9999'].includes(
					event.origin
				)
			)
				return;
