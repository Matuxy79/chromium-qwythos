...
# ---------------------------------------------------------------------------
# Container entry point for Qwythos.
...
if [[ -n "${SPACE_ID:-}" ]]; then
  ...
  if [[ -n "${ADMIN_USER_EMAIL:-}" && -n "${ADMIN_USER_PASSWORD:-}" ]]; then
    ...
    WEBUI_SECRET_KEY="${WEBUI_SECRET_KEY:-}" \
      uvicorn qwythos.main:app --host "$HOST" --port "$PORT" --forwarded-allow-ips "${FORWARDED_ALLOW_IPS:-*}" &
    webui_pid=$!
...
exec env WEBUI_SECRET_KEY="${WEBUI_SECRET_KEY:-}" \
  "$PYTHON_CMD" -m uvicorn qwythos.main:app \
    --host "$HOST" \
    --port "$PORT" \
