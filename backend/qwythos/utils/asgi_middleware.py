...
from qwythos.env import CUSTOM_API_KEY_HEADER
from qwythos.internal.db import ScopedSession
from qwythos.utils.auth import get_http_authorization_cred
...
class AuthTokenMiddleware:
    ...
    This is useful when Qwythos sits behind a reverse proxy that
    consumes the ``Authorization`` header for its own authentication —
    set the env var to a unique header (e.g. ``X-Qwythos-Key``) so
        ...
        if 'shared' in query_params and query_params['shared']:
            ...
            if text:
                ...
                if url_match:
                    ...
                    # YouTube URL.
                    from qwythos.retrieval.loaders.youtube import _parse_video_id
