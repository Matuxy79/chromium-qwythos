...
from qwythos.internal.db import get_async_db
from qwythos.models.automations import AutomationModel, AutomationRuns, Automations
...
from qwythos.utils.auth import create_token
from qwythos.utils.misc import parse_duration
from qwythos.utils.task import prompt_template
from qwythos.utils.terminals import get_terminal_server_url
...
async def scheduler_worker_loop(app) -> None:
    ...
    while True:
        try:
            ...
            # ── Timers ──
            try:
                from qwythos.utils.timers import claim_due_timers, execute_due_timer
...
async def _set_terminal_cwd(app, server_id: str, user, cwd: str, chat_id: str) -> None:
    ...
    Routes through the qwythos terminal proxy endpoint so that
    auth headers, orchestrator policy routing, and X-User-Id are
    handled correctly — same path the frontend uses.
    """
    import aiohttp
    from qwythos.env import AIOHTTP_CLIENT_SESSION_SSL
...
async def _check_calendar_alerts(app) -> None:
    ...
    """
    from qwythos.models.calendar import CalendarEvents, CalendarEventUpdateForm
    from qwythos.socket.main import sio
