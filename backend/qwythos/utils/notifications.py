...
from qwythos.events import EVENT_DEFINITIONS_BY_NAME, NOTIFICATION_EVENTS
from qwythos.models.config import Config
from qwythos.models.users import Users
from qwythos.retrieval.web.utils import validate_url
from qwythos.utils.webhook import post_webhook
...
async def test_target(user_id: str, target_id: str, app_name: str = 'Qwythos') -> dict[str, Any]:
    notifications = await _load_notifications(user_id)
    ...
    await _send_webhook(
        app_name,
        target,
        'This is a test notification from Qwythos.',
...
async def notify_target(
    user_id: str, message: str, target: str = '', title: str = '', app_name: str = 'Qwythos'
) -> dict[str, Any]:
    notifications = await _load_notifications(user_id)
...
async def dispatch_notification_event(app: Any, event: Any) -> None:
    ...
    from qwythos.events import event_user_ids

    app_name = getattr(getattr(app, 'state', None), 'WEBUI_NAME', 'Qwythos')
