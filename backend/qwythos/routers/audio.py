...
async def update_audio_config(request: Request, form_data: AudioConfigUpdateForm, user=Depends(get_admin_user)):
    ...
    # Local (in-process) speech-to-text is not supported in this build -- inference is
    # API-only. Set STT ENGINE to 'openai', 'deepgram', 'azure', or 'mistral'.
    ...
    try:
        ...
        if r is not None:
            try:
                ...
                detail = f'External: {e}'
        raise Exception(detail if detail else 'Qwythos: Server Connection Error')
