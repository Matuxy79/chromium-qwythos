...
from qwythos.events import EVENTS, publish_event, publish_model_provider_request_failed
...
from qwythos.internal.db import get_async_session
...
from qwythos.utils.access_control import check_model_access, has_connection_access, has_permission
from qwythos.utils.anthropic import get_anthropic_models, is_anthropic_url
from qwythos.utils.auth import get_admin_user, get_verified_user
from qwythos.utils.headers import get_custom_headers, include_user_info_headers
...
from qwythos.utils.session_pool import (
    cleanup_response,
    get_client_timeout,
    get_session,
    stream_wrapper,
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                ...
                detail=detail if detail else 'Qwythos: Server Connection Error',
            )
    ...
    if url_idx is None:
        ...
        async with aiohttp.ClientSession(
            ...
            try:
                ...
                raise HTTPException(status_code=500, detail='Qwythos: Server Connection Error')
            except Exception as e:
...
def _normalize_stored_item(item: dict) -> dict:
    ...
    Qwythos stores extra bookkeeping fields (``id``, ``status``,
...
async def proxy(path: str, request: Request, user=Depends(get_verified_user)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=r.status if r else 500,
            detail='Qwythos: Server Connection Error',
        )
    finally:
        if not streaming:
            await cleanup_response(r)
