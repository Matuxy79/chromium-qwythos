...
from qwythos.constants import ERROR_MESSAGES, TASKS
...
from qwythos.models.functions import Functions
from qwythos.models.messages import Messages
from qwythos.models.models import Models
from qwythos.models.users import Users
from qwythos.routers import (
    analytics,
    audio,
    auths,
    automations,
...
from qwythos.utils import logger
from qwythos.utils.access_control import has_permission
from qwythos.utils.actions import chat_action as chat_action_handler
from qwythos.utils.asgi_middleware import (
    AuthTokenMiddleware,
    CommitSessionMiddleware,
    RedirectMiddleware,
    WebsocketUpgradeGuardMiddleware,
)
from qwythos.utils.audit import AuditLevel, AuditLoggingMiddleware
from qwythos.utils.auth import (
    create_admin_user,
    decode_token,
    get_admin_user,
...
from qwythos.utils.chat import (
    chat_completed as chat_completed_handler,
)
from qwythos.utils.chat import (
    generate_chat_completion as chat_completion_handler,
)
from qwythos.utils.chat_id import (
    get_temporary_chat_session_id,
    is_saved_chat_id,
    is_temporary_chat_id,
)
from qwythos.utils.chat_variables import (
    normalize_chat_variables,
)
from qwythos.utils.embeddings import generate_embeddings
from qwythos.utils.json_response import apply_orjson_http_json
from qwythos.utils.logger import start_logger
from qwythos.utils.middleware import (
    background_tasks_handler,
    build_chat_response_context,
    process_chat_payload,
    process_chat_response,
)
from qwythos.utils.model_ids import strip_provider_model_prefix
from qwythos.utils.models import (
    check_model_access,
    get_all_base_models,
    get_all_models,
    get_filtered_models,
...
from qwythos.utils.plugin import install_tool_and_function_dependencies
from qwythos.utils.redis import get_redis_client
from qwythos.utils.security_headers import SecurityHeadersMiddleware
from qwythos.utils.session_pool import cleanup_response, get_session, stream_wrapper
from qwythos.utils.tools import set_terminal_servers, set_tool_servers
...
async def lifespan(app: FastAPI):
    ...
    from qwythos.utils.automations import scheduler_worker_loop
    ...
    from qwythos.utils.session_pool import close_session
...
if ENABLE_OTEL:
    from qwythos.utils.telemetry.setup import setup as setup_opentelemetry
...
async def initialize_runtime_config(app: FastAPI):
    ...
    from qwythos.utils.access_control import migrate_access_control
...
# `qwythos.utils.asgi_middleware` for the rationale.
...
async def _set_direct_model(request: Request, model_item: dict, user) -> None:
    ...
    if knowledge_items:
        from qwythos.utils.access_control.files import get_accessible_folder_files
                ...
                if is_new_chat:
                    ...
                    if user_message and user_message.get('id'):
                        ...
                        if not getattr(request.state, 'internal', False) and not (user_message.get('meta') or {}).get(
                            ...
                            try:
                                from qwythos.utils.timers import cancel_timers_for_chat
        ...
        try:
            ...
            try:
                ...
                if (
                    ...
                    from qwythos.utils.subagents import process_pending_internal_messages
...
from qwythos.utils.anthropic import (
...
async def generate_messages(
    ...
    All models configured in Qwythos are accessible via this endpoint.
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout, trust_env=True) as session:
            async with session.get(
                'https://api.github.com/repos/qwythos/qwythos/releases/latest',
                ssl=AIOHTTP_CLIENT_SESSION_SSL,
            ) as response:
...
async def get_current_usage(user=Depends(get_verified_user)):
    """
    Get current usage statistics for Qwythos.
