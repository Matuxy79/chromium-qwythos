...
def run_migrations():
    ...
    try:
        ...
        alembic_cfg = AlembicConfig(QWYTHOS_DIR / 'alembic.ini')

        migrations_path = QWYTHOS_DIR / 'migrations'
...
STATIC_DIR = Path(os.getenv('STATIC_DIR', QWYTHOS_DIR / 'static')).resolve()
...
if CUSTOM_NAME:
    try:
        r = requests.get(f'https://api.qwythos.com/api/v1/custom/{CUSTOM_NAME}')
        data = r.json()
        if r.ok:
            if 'logo' in data:
                WEBUI_FAVICON_URL = url = (
                    f'https://api.qwythos.com{data["logo"]}' if data['logo'][0] == '/' else data['logo']
                )
            ...
            if 'splash' in data:
                url = f'https://api.qwythos.com{data["splash"]}' if data['splash'][0] == '/' else data['splash']
...
if ENV == 'prod':
    if OLLAMA_BASE_URL == '/ollama' and not K8S_FLAG:
        if USE_OLLAMA_DOCKER.lower() == 'true':
            # if you use all-in-one docker container (Qwythos + Ollama)
            # with the docker build arg USE_OLLAMA=true (--build-arg="USE_OLLAMA=true") this only works with http://localhost:11434
            OLLAMA_BASE_URL = 'http://localhost:11434'
        else:
            OLLAMA_BASE_URL = 'http://host.docker.internal:11434'
    elif K8S_FLAG:
        OLLAMA_BASE_URL = 'http://ollama-service.qwythos.svc.cluster.local:11434'
...
MILVUS_COLLECTION_PREFIX = os.getenv('MILVUS_COLLECTION_PREFIX', 'qwythos')
...
QDRANT_COLLECTION_PREFIX = os.getenv('QDRANT_COLLECTION_PREFIX', 'qwythos')
...
ELASTICSEARCH_INDEX_PREFIX = os.getenv('ELASTICSEARCH_INDEX_PREFIX', 'qwythos_collections')
# Pgvector
...
PINECONE_INDEX_NAME = os.getenv('PINECONE_INDEX_NAME', 'qwythos-index')
...
VALKEY_COLLECTION_PREFIX = os.getenv('VALKEY_COLLECTION_PREFIX', 'qwythos')
...
if JWT_EXPIRES_IN == '-1':
    log.warning(
        ...
        '    See: https://docs.qwythos.com/reference/env-configuration\n'
    )
