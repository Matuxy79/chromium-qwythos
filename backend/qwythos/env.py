...
# QWYTHOS_DIR should be the directory where env.py resides (qwythos/)
QWYTHOS_DIR = ENV_FILE_PATH.parent

# BACKEND_DIR is the parent of QWYTHOS_DIR (backend/)
BACKEND_DIR = QWYTHOS_DIR.parent

# BASE_DIR is the parent of BACKEND_DIR (qwythos-dev/)
...
if FROM_INIT_PY:
    PACKAGE_DATA = {'version': importlib.metadata.version('qwythos')}
else:
    try:
...
# to orjson. Faster, but stricter: see qwythos/utils/json_codec.py for the differences.
...
try:
    ...
    changelog_content = (pkgutil.get_data('qwythos', 'CHANGELOG.md') or b'').decode()
...
if FROM_INIT_PY:
    NEW_DATA_DIR = Path(os.getenv('DATA_DIR', QWYTHOS_DIR / 'data')).resolve()
    ...
    if DATA_DIR.exists() and DATA_DIR != NEW_DATA_DIR:
        ...
        shutil.make_archive(DATA_DIR.parent / 'qwythos_data', 'zip', DATA_DIR)

        # Remove the old data directory
        shutil.rmtree(DATA_DIR)

    DATA_DIR = Path(os.getenv('DATA_DIR', QWYTHOS_DIR / 'data'))

STATIC_DIR = Path(os.getenv('STATIC_DIR', QWYTHOS_DIR / 'static'))

FONTS_DIR = Path(os.getenv('FONTS_DIR', QWYTHOS_DIR / 'static' / 'fonts'))

FRONTEND_BUILD_DIR = Path(os.getenv('FRONTEND_BUILD_DIR', BASE_DIR / 'build')).resolve()

if FROM_INIT_PY:
    FRONTEND_BUILD_DIR = Path(os.getenv('FRONTEND_BUILD_DIR', QWYTHOS_DIR / 'frontend')).resolve()
...
REDIS_KEY_PREFIX = os.getenv('REDIS_KEY_PREFIX', 'qwythos')
...
# but is scoped to Qwythos to avoid interfering with system-level settings.
...
def _parse_ssl_env(value: str) -> 'bool | _ssl.SSLContext':
    ...
    This allows users with corporate or internal CAs to point Qwythos
...
if WEBUI_AUTH and WEBUI_SECRET_KEY == '':
    raise SystemExit(
        ...
        'start_windows.bat (Windows), or `qwythos serve`.\n'
        ...
        'See https://docs.qwythos.com/reference/env-configuration#webui_secret_key'
    )
...
# Useful when Qwythos sits behind a reverse proxy / API gateway that
# already uses the Authorization header for its own authentication — set
# this to a unique header (e.g. 'X-Qwythos-Key') so the middleware
...
# compatible APIs for endpoints not natively handled by Qwythos.
...
# Allows external apps to exchange OAuth tokens for Qwythos tokens
...
WEBUI_NAME = os.getenv('WEBUI_NAME', 'Qwythos')
...
FORWARD_USER_INFO_HEADER_USER_NAME = os.getenv('FORWARD_USER_INFO_HEADER_USER_NAME', 'X-Qwythos-User-Name')
FORWARD_USER_INFO_HEADER_USER_ID = os.getenv('FORWARD_USER_INFO_HEADER_USER_ID', 'X-Qwythos-User-Id')
FORWARD_USER_INFO_HEADER_USER_EMAIL = os.getenv('FORWARD_USER_INFO_HEADER_USER_EMAIL', 'X-Qwythos-User-Email')
FORWARD_USER_INFO_HEADER_USER_ROLE = os.getenv('FORWARD_USER_INFO_HEADER_USER_ROLE', 'X-Qwythos-User-Role')
FORWARD_SESSION_INFO_HEADER_MESSAGE_ID = os.getenv('FORWARD_SESSION_INFO_HEADER_MESSAGE_ID', 'X-Qwythos-Message-Id')
FORWARD_SESSION_INFO_HEADER_CHAT_ID = os.getenv('FORWARD_SESSION_INFO_HEADER_CHAT_ID', 'X-Qwythos-Chat-Id')

# If set while ENABLE_FORWARD_USER_INFO_HEADERS is True, send one signed HS256 JWT
# (FORWARD_USER_INFO_HEADER_JWT) instead of separate X-Qwythos-User-* headers.
FORWARD_USER_INFO_HEADER_JWT_SECRET = (os.environ.get('FORWARD_USER_INFO_HEADER_JWT_SECRET') or '').strip() or None
FORWARD_USER_INFO_HEADER_JWT = os.environ.get('FORWARD_USER_INFO_HEADER_JWT', 'X-Qwythos-User-Jwt')
try:
...
OTEL_SERVICE_NAME = os.getenv('OTEL_SERVICE_NAME', 'qwythos')
