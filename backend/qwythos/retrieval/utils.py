...
from qwythos.retrieval.vector.main import GetResult, SearchResult
from qwythos.retrieval.web.utils import get_web_loader
from qwythos.utils.access_control.files import get_owner_accessible_folder_files, has_access_to_file
...
def build_loader_from_config(request, config: dict):
    ...
    from qwythos.retrieval.loaders.main import Loader
...
def _get_content_from_url_sync(request, url: str, loader_config):
    from qwythos.retrieval.web.utils import validate_url, _SSRFSafeAdapter
...
# Anything else cannot be a real Qwythos collection and could break out of
...
class RerankCompressor(BaseDocumentCompressor):
    ...
    async def acompress_documents(
        ...
        if reranking:
            ...
            d = np.asarray(document_embedding, dtype=np.float64)
