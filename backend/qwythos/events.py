...
from qwythos.env import ENABLE_PLUGINS, VERSION
...
async def dispatch_webhook_event(app: Any, event: Event) -> None:
    name = getattr(getattr(app, 'state', None), 'WEBUI_NAME', 'Qwythos')
    subject = event.subject or {}
...
def schedule_notification_dispatch(app: Any, event: Event) -> None:
    try:
        from qwythos.utils.notifications import dispatch_notification_event
...
async def dispatch_event_functions(
    ...
    from qwythos.utils.plugin import get_function_module_from_cache
